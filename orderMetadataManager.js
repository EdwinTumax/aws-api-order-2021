...
order : {
    ...
    deliveryStatus: READY_FOR_DELIVERY / DELIVERED
    timestamp: timestamp
}
 */
...
module.exports.saveCompletedOrder = order => {
    ...
    console.log(order);
    console.log(process.env.COMPLETED_ORDER_TABLE);
    const params = {
...
module.exports.deliverOrder = orderId => {
    console.log(`Iniciando el proceso - Enviar orden con número ${orderId}`);
    const params = {
		TableName: process.env.COMPLETED_ORDER_TABLE,
		Key: {
			orderId
...
module.exports.getOrder = orderId => {
    ...
    return dynamo.get(params).promise().then(item => { return item.Item; });
}